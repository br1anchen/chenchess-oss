...
function isBroadRecordTypeReference(type: ESTree.TSTypeReference): boolean {
...
function isBroadIndexSignatureRecord(type: ESTree.TSTypeLiteral): boolean {
...
function isBroadRecordType(type: ESTree.TSType): boolean {
  ...
  return (
    unwrapped.type === "TSTypeLiteral" && isBroadIndexSignatureRecord(unwrapped)
  )
}
...
function knownAssertionEvidence(
  expression: ESTree.TSAsExpression | ESTree.TSTypeAssertion,
): KnownValueEvidence | null {
  if (broadTypeKind(expression.typeAnnotation) !== null) return null
  return { type: expression.typeAnnotation }
}

function isKnownConstructedValue(expression: ESTree.Expression): boolean {
  return (
...
function knownBoundValueEvidence(
  variable: Variable,
...
function knownValueEvidence(
  ...
  if (isKnownConstructedValue(unwrapped)) return { type: null }
